...
function App() {
  ...
  const loadVideoAnalysis = async (videoId) => {
    ...
    try {
      const headers = user?.id ? { 'X-Clerk-User-Id': user.id } : {}
      const res = await fetch(`${API_BASE}/api/videos/${videoId}/analysis/`, { headers })
      if (!res.ok) return
      const data = await res.json()
      const payload = data.payload || {}
      const analyses = payload.analyses || []
      setVideoAnalysisByPostId((current) => ({ ...current, [videoId]: analyses }))
      return analyses
    } catch (err) {
      // ignore
    }
  ...
  const renderDetailPage = () => {
    ...
    return (
      <section className="page-content video-detail-page">
        ...
        {/* AI analysis section */}
        {videoAnalysisByPostId[video.id] && videoAnalysisByPostId[video.id].length > 0 ? (
          <section className="ai-analysis">
            <h3>AI Analysis</h3>
            {videoAnalysisByPostId[video.id].map((a) => (
              <div key={a.id} className="ai-analysis-block">
                <div className="ai-analysis-meta">Generated by {a.generated_by} at {new Date(a.created_at).toLocaleString()}</div>
                {a.analysis && a.analysis.summary ? <p className="ai-summary">{a.analysis.summary}</p> : null}
                {a.analysis && a.analysis.metadata && a.analysis.metadata.transcript ? (
                  <div className="ai-transcript">
                    <strong>Transcript</strong>
                    <p>{a.analysis.metadata.transcript}</p>
                  </div>
                ) : null}
                {a.analysis && a.analysis.metadata && Array.isArray(a.analysis.metadata.detections) && a.analysis.metadata.detections.length > 0 ? (
                  <div className="ai-detections">
                    <strong>Detections</strong>
                    <ul>
                      {a.analysis.metadata.detections.map((d, idx) => (
                        <li key={idx}>{d.label} ({Math.round((d.confidence||0)*100)}%)</li>
                      ))}
                    </ul>
                  </div>
                ) : null}
